...
function cleanLogs {
    rm -rf /users/$(whoami)/eeflink/flink-simplified/flink-dist/target/flink-1.14.0-bin/flink-1.14.0/log/*.log*
    ssh ${IPSOURCE} rm -rf /users/$(whoami)/eeflink/flink-simplified/flink-dist/target/flink-1.14.0-bin/flink-1.14.0/log/*.log*
    ssh ${IPMAPPER} rm -rf /users/$(whoami)/eeflink/flink-simplified/flink-dist/target/flink-1.14.0-bin/flink-1.14.0/log/*.log*
    ssh ${IPSINK} rm -rf /users/$(whoami)/eeflink/flink-simplified/flink-dist/target/flink-1.14.0-bin/flink-1.14.0/log/*.log*
}
...
function dynamic {
	...
	for i in `seq ${BEGIN_ITER} 1 $NITERS`; do
		...
		for pol in $MPOLICY; do
		    ...
		    sleep 1			    
 		    echo "🟢🟢 [INFO] FINISH ${MQUERY}_cores${NCORES}_frate${fr}_fratetype_${FLINK_RATE_TYPE}_fbuff-1_itr1_${pol}dvfs1_source${nsrc}_mapper${nmapper}_sink${nsink}_repeat${i} [INFO] 🟢🟢"
		done
	    done
	done    
    done
